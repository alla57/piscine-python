...
def main():
    ...
    This script takes a single string as a command-line argument and encodes it into Morse code.
    ...
    - If the number of arguments is not 1 or the argument is not a string, or contains unhandled characters, an AssertionError is raised.
    """
    morseDict = {
    'A': '.-',     'B': '-...',   'C': '-.-.', 
    'D': '-..',    'E': '.',      'F': '..-.',
    ...
    try:
        ...
        for c in sys.argv[1]:
            if not c.isalnum():
                if c != ' ':
                    raise AssertionError("Characters not supported! Please use only Alphanumeric and space characters")
